        ...
        rows.append(
            {
                ...
                "vib_low_frequency_peak_g": round(_fft_amplitude(vib, sampling_hz, 4.0, bandwidth_hz=1.0), 6),
                "vib_friction_peak_g": round(_fft_amplitude(vib, sampling_hz, 2.5, bandwidth_hz=0.7), 6),
                ...
                "current_load_normalized_a": round(float(np.mean(current)) / max(float(np.mean(load)) / 100.0, 0.25), 5),
